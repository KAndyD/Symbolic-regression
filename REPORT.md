...
Вариант 5: эволюционный поиск формул для символьной регрессии с контролем разрастания деревьев. Заявка подготовлена в `PROPOSAL.md`. Компоненты: генератор данных → синтаксические деревья → безопасный интерпретатор → GP → отбор по validation → итоговая test-оценка → анализ качества и сложности.
