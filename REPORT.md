...
## Краткая заявка на тему

В качестве темы я выбрал эволюционный поиск математической формулы по набору данных. Задача состоит в том, чтобы подобрать выражение, которое достаточно точно описывает заданную зависимость, но при этом не становится слишком большим и сложным.

Отдельное решение представляется в виде дерева выражения. В нём могут использоваться переменная \(x\), числовые константы и операции сложения, вычитания, умножения, защищённого деления и синуса. Чтобы деревья не разрастались бесконечно, ограничиваются их глубина и количество узлов.

В работе планируется сравнить два варианта генетического программирования: обычный алгоритм и алгоритм со штрафом за размер формулы. Для честного сравнения им задаётся одинаковое количество вычислений. Данные разделяются на обучающую, проверочную и тестовую выборки. Результат оценивается по среднеквадратичной ошибке, размеру найденной формулы и устойчивости алгоритма в серии из 30 запусков.

Основные возможные проблемы — переобучение, чрезмерное разрастание деревьев и появление численно нестабильных выражений. Для их ограничения используются отдельная проверочная выборка, ограничение размера дерева и защищённое деление.
